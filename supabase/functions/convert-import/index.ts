...
function factsBlock(args: {
  ...
  if (args.mode === 'triage') {
    lines.push(
      ...
      '',
      'WHAT IS WORTH A QUESTION. A source category that maps two ways and would file a lot of rows',
      'wrongly is worth one - "Scommesse" could be Leisure or Other, and hundreds of euros hang on',
      'it. Ask about those, at most six of them, biggest first, each naming the source category in',
      'quotes and offering my closest categories as options. What is NOT worth one: anything you',
      'are already fairly sure of. "Attività fisica goes to Sport, right?" spends a question on an',
      'answer you have; decide it, and put it in "notes" as one line - I read the notes before I',
      'commit and can send the whole thing back if you got it wrong. A screen of confirmations',
      'reads as an app that cannot make up its mind, and every one I have to tap is a tax on an',
      'import I asked you to do for me.',
    );
...
function apiAddendum(language: 'en' | 'it'): string {
  ...
  return [
    ...
    '   A question is for a real fork, not for reassurance. Where one of my categories is the',
    '   obvious home for a source category, take it and put the mapping in "notes" - a question I',
    '   answer with "yes" was a line you could have written. Where a source category genuinely',
    '   maps two ways and a lot of rows hang on it, ask, naming it in quotes with my closest',
    '   categories as options. Never ask about a subcategory or a description: those are yours to',
    '   decide and mine to correct afterwards. Names inside descriptions ("lunch with Marco") are',
    '   people I was with, not payers or columns: on a file with no per-person columns, never ask',
    '   which of them is me.',
...
- OGNI riga di un file diviso \xE8 una SPESA. I soldi che mi tornano indietro non sono un'entrata: sono il motivo per cui la mia quota \xE8 PI\xD9 PICCOLA del costo della riga, e sono gi\xE0 contati nel momento in cui prendo la mia quota invece di quello che ho anticipato. Una riga di vacanza che arriva come entrata sotto una categoria di stipendio \xE8 il segno pi\xF9 chiaro che la regola dei saldi \xE8 stata saltata: su un saldo di +144,82 con un costo di 181,03 la mia quota \xE8 36,21, non 144,82 e non un'entrata.
...
- EVERY row of a split file is an EXPENSE. Money coming back to me on one is not income - it is the reason my share is SMALLER than the row's cost, and it is already accounted for the moment I take my share instead of what I paid. A row of a holiday that arrives as income under a salary category is the plainest sign the balance rule was skipped: on a +144.82 balance against a 181.03 cost my share is 36.21, not 144.82 and not income.
