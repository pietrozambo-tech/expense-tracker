// Runs scripts/bulkedit-test/check.ts against the real lib/bulkEdit.ts and the
...
if (!esbuild) {
  console.error('bulkedit-test: esbuild binary not found under node_modules/.pnpm');
  process.exit(1);
}

const tmp = mkdtempSync(join(tmpdir(), 'bulkedit-test-'));
let failed = 0;
try {
  const bundle = join(tmp, 'check.mjs');
  execFileSync(process.execPath, [
    esbuild, join(root, 'scripts/bulkedit-test/check.ts'),
