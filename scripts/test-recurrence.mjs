// Runs the recurrence scenarios in scripts/recurrence-test/scenarios.ts against
// the real src/app/lib/recurrence.ts.
//
...
if (!esbuild) {
  console.error('recurrence-test: esbuild binary not found under node_modules/.pnpm');
  process.exit(1);
}

const tmp = mkdtempSync(join(tmpdir(), 'recurrence-test-'));
try {
  mkdirSync(join(tmp, 'lib'));
  mkdirSync(join(tmp, 'utils'));
  copyFileSync(join(root, 'src/app/types.ts'), join(tmp, 'types.ts'));
  for (const f of ['recurrence.ts', 'dates.ts', 'fx.ts', 'currencyData.ts']) {
  ...
  copyFileSync(join(root, 'scripts/recurrence-test/scenarios.ts'), join(tmp, 'scenarios.ts'));
