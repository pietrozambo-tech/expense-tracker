// Runs scripts/periods-test/check.ts against the real lib/periods.ts and the
...
if (!esbuild) {
  console.error('periods-test: esbuild binary not found under node_modules/.pnpm');
  process.exit(1);
}

const tmp = mkdtempSync(join(tmpdir(), 'periods-test-'));
let failed = 0;
try {
  const bundle = join(tmp, 'check.mjs');
  execFileSync(process.execPath, [
    esbuild, join(root, 'scripts/periods-test/check.ts'),
