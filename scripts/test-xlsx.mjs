// Runs scripts/xlsx-test/check.ts against the real lib/trips.ts and the
...
if (!esbuild) {
  console.error('xlsx-test: esbuild binary not found under node_modules/.pnpm');
  process.exit(1);
}
...
try {
  ...
  execFileSync(process.execPath, [
    esbuild, join(root, 'scripts/xlsx-test/check.ts'),
