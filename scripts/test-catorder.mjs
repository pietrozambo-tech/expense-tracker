// Runs scripts/catorder-test/check.ts against the real lib/categoryOrder.ts and the
...
if (!esbuild) {
  console.error('catorder-test: esbuild binary not found under node_modules/.pnpm');
  process.exit(1);
}

const tmp = mkdtempSync(join(tmpdir(), 'catorder-test-'));
let failed = 0;
try {
  const bundle = join(tmp, 'check.mjs');
  execFileSync(process.execPath, [
    esbuild, join(root, 'scripts/catorder-test/check.ts'),
