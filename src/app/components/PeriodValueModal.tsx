import { X } from 'lucide-react';
import { useBackClose } from '../lib/useBackClose';

interface PeriodValueModalProps {
  isOpen: boolean;
  periodType: 'month' | 'quarter' | 'year';
  selectedPeriodValue: string;
  onClose: () => void;
  onSelectPeriodValue: (periodValue: string) => void;
}

export function PeriodValueModal({ 
  isOpen, 
  periodType,
  selectedPeriodValue, 
  onClose, 
  onSelectPeriodValue 
}: PeriodValueModalProps) {
  useBackClose(isOpen, onClose);
  if (!isOpen) return null;

  // Get available options based on period type
  const getOptions = () => {
    const now = new Date();
    const currentYear = now.getFullYear();
    
    switch (periodType) {
      case 'month':
        return [
          'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
          'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'
        ];
      case 'quarter':
        return ['Q1', 'Q2', 'Q3', 'Q4'];
      case 'year':
        // Generate last 5 years and current year
        return Array.from({ length: 6 }, (_, i) => String(currentYear - i));
    }
  };

  const options = getOptions();

  // Get modal title based on period type
  const getTitle = () => {
    switch (periodType) {
      case 'month': return 'Select Month';
      case 'quarter': return 'Select Quarter';
      case 'year': return 'Select Year';
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/30" />
      
      {/* Modal */}
      <div 
        className="relative w-full bg-white rounded-t-3xl shadow-xl max-w-[430px] mx-auto"
        onClick={(e) => e.stopPropagation()}
        style={{ maxHeight: '85vh' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-neutral-100">
          <h2 className="font-semibold text-neutral-900">{getTitle()}</h2>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-neutral-100 transition-colors"
          >
            <X size={20} className="text-neutral-500" />
          </button>
        </div>

        {/* Options List */}
        <div className="overflow-y-auto px-6 py-4" style={{ maxHeight: 'calc(85vh - 80px)' }}>
          {options.map((option) => (
            <button
              key={option}
              onClick={() => {
                onSelectPeriodValue(option);
                onClose();
              }}
              className="w-full flex items-center justify-between py-3.5 transition-colors hover:bg-neutral-50 rounded-lg px-3 -mx-3"
            >
              <span className={`text-sm ${selectedPeriodValue === option ? 'font-medium text-blue-600' : 'text-neutral-900'}`}>
                {option}
              </span>
              {selectedPeriodValue === option && (
                <div className="w-5 h-5 rounded-full bg-blue-600 flex items-center justify-center">
                  <svg width="12" height="10" viewBox="0 0 12 10" fill="none">
                    <path d="M1 5L4.5 8.5L11 1" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              )}
            </button>
          ))}
        </div>
      </div>
    </div>
  );
}
