...
interface YearFilterModalProps {
  isOpen: boolean;
  selectedYear: string;
  availableYears: string[]; // Years that have actual expense data
  onClose: () => void;
  onSelectYear: (year: string) => void;
}

export function YearFilterModal({ 
  isOpen, 
  selectedYear,
  availableYears, 
  onClose, 
  onSelectYear 
}: YearFilterModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-neutral-100">
          <h2 className="font-semibold text-neutral-900">Select Year</h2>
          <button
            onClick={onClose}
          ...
          {availableYears.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {availableYears.map((year) => (
                ...
                >
                  <span className={`${selectedYear === year ? 'font-semibold text-blue-600' : 'font-medium text-neutral-900'}`} style={{ fontSize: '14px' }}>
                    {year}
                  </span>
                  {selectedYear === year && (
